...
#[pyclass(module = "lightllm.server.httpserver_for_pd_master.pd_selector._pd_tree_rust")]
#[derive(Debug, Clone)]
...
#[pymodule]
fn _pd_tree_rust(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<Tree>()?;
    m.add_class::<PrefixMatchResult>()?;
    Ok(())
}
