...
from lightllm.common.mem_manager import BaseAllocator, MemoryManager
...
class LayerCacheMemoryManager(BaseAllocator):
    ...
    def _init_buffers(self, size, dtype, shape):
        self.buffer = torch.zeros(
            (self.layer_num, size + 1, *shape), dtype=dtype, device="cuda")
...
class Qwen3NextMemoryManager(MemoryManager, HaveStateBuffer):
    def __init__(
        ...
        logger.info(f"Conv state shape: {conv_state_shape}")
        logger.info(f"Ssm state shape: {ssm_state_shape}")
        logger.info(f"Linear attention layer num: {self.linear_attn_layer_num}")
        logger.info(f"Full attention layer num: {self.full_attn_layer_num}")
        logger.info(f"Mtp layer num: {self.mtp_layer_num}")
        logger.info(f"Full attention interval: {self.full_attention_interval}")
        ...
        logger.info(
            ...
            f"Conv state use : {self.conv_state_mem_manager.get_cell_size() * linear_attn_cache_size / 1024 ** 3} GB Memory.\n"
            f"Ssm state use : {self.ssm_state_mem_manager.get_cell_size() * linear_attn_cache_size / 1024 ** 3} GB Memory.\n"
        )
    ...
    def _init_buffers(self, size, dtype, head_num, head_dim, layer_num):
        # kv_buffer = [None, None, None, kv_cache, None, None, None, kv_cache, ..., None, kv_cache, mtp_kv_cache, mtp_kv_cache]
        self.kv_buffer = [None for _ in range(self.layer_num)]
        for layer_id in range(self.full_attn_layer_num):
            self.kv_buffer[(layer_id + 1)*self.full_attention_interval - 1] = torch.empty((size + 1, 2 * head_num, head_dim), dtype=dtype, device="cuda")
