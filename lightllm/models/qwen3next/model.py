...
class Qwen3NextTpPartModel(Qwen3MOEModel):
    ...
    def _init_infer_layer(self):
        ...
        # Create shared buffer pool for all layers
        buffer_pool = Qwen3NextBufferPool()
        ...
        # Attach buffer pool to all layers
        for layer in self.layers_infer:
            layer.buffer_pool = buffer_pool

        # Store reference to buffer pool for release_all() calls
        self.buffer_pool = buffer_pool

    @override
    def _context_forward(self, infer_state):
        """Override to add buffer pool release after prefill."""
        model_output = super()._context_forward(infer_state)
        # Release all buffers after prefill pass
    ...
    def _token_forward(self, infer_state):
        """Override to add buffer pool release after decode."""
        model_output = super()._token_forward(infer_state)
        # Release all buffers after decode pass
        if hasattr(self, "buffer_pool") and self.buffer_pool is not None:
            self.buffer_pool.release_all()
        return model_output
