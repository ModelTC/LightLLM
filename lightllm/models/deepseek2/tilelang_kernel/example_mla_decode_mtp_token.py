...
def mla_decode_tilelang2(mtp_size, batch, h_q, h_kv, max_seqlen_pad, dv, dpe, block_N, block_H, num_split,
    ...
    def flash_mla_kernel(
        ...
        with T.Kernel(real_batch, 1, threads=256) as (bx, by):
            ...
            kv_idx = T.alloc_shared([block_N], "int32") # (64)
            ...
            for kr in T.Pipelined(loop_range, num_stages=2):
                ...
                T.copy(BLOCK_TABLE[b_index_last, k * block_N: (k + 1) * block_N], kv_idx)
                for i, j in T.Parallel(block_N, dv):
                    KV_shared[i, j] = KV[kv_idx[i], cur_kv_head, j]
                for i, j in T.Parallel(block_N, dpe):
                    K_pe_shared[i, j] = K_pe[kv_idx[i], cur_kv_head, j]
                T.clear(acc_s)
                T.gemm(
...
def run_tilelang_mtp2(mtp_size, q, block_table, blocked_k, max_seqlen_pad, block_size, b, s_q, cache_seqlens,
    ...
    # with open(f"examples/deepseek_mla/example_mla_decode_token_mtp_{mtp_size}.cu", "w") as f:
