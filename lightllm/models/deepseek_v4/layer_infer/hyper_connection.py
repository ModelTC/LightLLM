...
def hc_pre(residual, hc_fn, hc_scale, hc_base, rms_eps, hc_eps, sinkhorn_iters, norm_weight, norm_eps):
    ...
    (x[T,dim], residual, post_mix[T,hc,1], res_mix[T,hc,hc])."""
    post_mix, res_mix, x = torch.ops.vllm.mhc_pre(
        residual=residual,
        fn=hc_fn,
        hc_scale=hc_scale,
        hc_base=hc_base,
    ...
    x = rmsnorm_forward(x, norm_weight, norm_eps)
...
def hc_fused_post_pre(
    ...
    """hc_post of the previous sub-layer fused with hc_pre of the next one.
...
def hc_post(x, residual, post_mix, res_mix):
    ...
    return torch.ops.vllm.mhc_post(x, residual, post_mix, res_mix)
