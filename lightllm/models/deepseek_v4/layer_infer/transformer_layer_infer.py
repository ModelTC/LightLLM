...
class DeepseekV4TransformerLayerInfer(Deepseek3_2TransformerLayerInfer):
    ...
    def overlap_tpsp_token_forward(
        ...
        (
            ...
            handle0,
            dispatch_hook0,
        ) = experts.low_latency_dispatch_with_topk(x0, indices0, weights0)
        ...
        (
            ...
            handle1,
            dispatch_hook1,
        ) = experts.low_latency_dispatch_with_topk(x1, indices1, weights1)
    ...
    def _get_qkv(
        ...
        if infer_state.is_prefill:
            ...
            q = self.alloc_tensor(
                (T, self.flashmla_q_head_num_, self.head_dim_),
                dtype=q_in.dtype,
                device=q_in.device,
            )
    ...
    def _context_attention_wrapper_run(
        ...
        if torch.cuda.is_current_stream_capturing():
            ...
            o = self.alloc_tensor(
                (q.shape[0], self.tp_q_head_num_, self.head_dim_),
                dtype=q.dtype,
                device=q.device,
            )
...
class DeepseekV4IndexInfer(BaseLayerInfer):
    ...
    def _c4_indices(self, infer_state: DeepseekV4InferStateInfo, idx_q_fp8, weights, positions):
        ...
        if cached is None:
            from ..triton_kernel.gather_c4_indexer_k_dsv4 import (
                build_c4_indexer_page_table,
            )
