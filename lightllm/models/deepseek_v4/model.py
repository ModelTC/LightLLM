...
class DeepseekV4TpPartModel(LlamaTpPartModel):
    ...
    def _init_mem_manager(self):
        if self.max_total_token_num is None:
            # _init_weights already reserves model weights, but DSV4 persistent runtime
            # allocations were previously initialized after this cuda.mem_get_info() profile.
            # Make them visible to the automatic KV capacity sample.
    ...
    def _init_flashmla_prefill_workspace(self, real_q_head_num: int, padded_q_head_num: int):
    ...
    def _init_att_backend(self):
        if getattr(self, "_dsv4_att_backend_initialized", False):
            return
        ...
        self._init_flashmla_prefill_workspace(real_q_head_num=real_q_head_num, padded_q_head_num=padded_q_head_num)
        ...
        self._dsv4_att_backend_initialized = True
        return
    ...
    def _init_to_get_rotary(self):
        ...
        if not hasattr(self, "_freqs_cis_sliding"):
            self._init_dsv4_rotary_tables()
        self._bind_dsv4_rotary_tables()
        return

    def _init_dsv4_rotary_tables(self):
        cfg = self.config
