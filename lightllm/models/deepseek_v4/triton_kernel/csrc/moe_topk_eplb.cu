...
__global__ void moe_topk_eplb_kernel(
  ...
  if constexpr (IsHash) {
    ...
    for (int j = 0; j < 8; ++j) {
      ...
      local_choice[j] = score(row[id]) + bias[id];
    }
    #pragma unroll
    for (int k = 0; k < 8; ++k) {
      if (k >= topk) break;
...
void dispatch(const torch::Tensor& logits, const torch::Tensor& bias, const torch::Tensor& input_ids,
  ...
  if (is_hash && single_token) {
    launch<true, RecordLoad, ReturnLogical, true>(logits, bias, input_ids, tid2eid, weights, physical_ids,
                                                   logical_ids, logical_to_physical, replica_count, counter,
                                                   sample_index, routed_scaling_factor);
  } else if (is_hash) {
    launch<true, RecordLoad, ReturnLogical, false>(logits, bias, input_ids, tid2eid, weights, physical_ids,
                                                    logical_ids, logical_to_physical, replica_count, counter,
                                                    sample_index, routed_scaling_factor);
  } else if (single_token) {
    launch<false, RecordLoad, ReturnLogical, true>(logits, bias, input_ids, tid2eid, weights, physical_ids,
                                                    logical_ids, logical_to_physical, replica_count, counter,
                                                    sample_index, routed_scaling_factor);
  } else {
    launch<false, RecordLoad, ReturnLogical, false>(logits, bias, input_ids, tid2eid, weights, physical_ids,
                                                     logical_ids, logical_to_physical, replica_count, counter,
                                                     sample_index, routed_scaling_factor);
  }
}
