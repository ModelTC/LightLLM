...
class Qwen3_5DFlashModel(Qwen3DFlashModel):
    ...
    def _init_config(self):
        ...
        for key in ("target_layer_ids", "mask_token_id"):
            if key not in self.config and key in dflash_config:
                self.config[key] = dflash_config[key]
    ...
    def _init_mem_manager(self):
        ...
        target_linear_config = target_mem_manager.linear_config

        draft_head_dim = self.config.get("head_dim")
        if draft_head_dim is None:
            draft_head_dim = self.config["hidden_size"] // self.config["num_attention_heads"]
        ...
        assert int(draft_head_dim) == int(target_mem_manager.head_dim) and draft_kv_heads == target_kv_heads, (
