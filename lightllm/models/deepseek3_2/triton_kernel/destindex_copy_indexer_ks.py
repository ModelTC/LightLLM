...
def test_destindex_copy_indexer_ks():
    ...
    fp8_match_seq = torch.allclose(k_fp8_out_seq.to(torch.float32), k_fp8_seq.to(torch.float32), atol=0, rtol=0)
    scale_match_seq = torch.allclose(k_scale_out_seq, k_scale_seq.squeeze(-1), atol=1e-6, rtol=1e-5)
    ...
    fp8_match_single = torch.allclose(
        k_fp8_out_single.to(torch.float32), k_fp8_single.to(torch.float32), atol=0, rtol=0
    )
    scale_match_single = torch.allclose(k_scale_out_single, k_scale_single.squeeze(-1), atol=1e-6, rtol=1e-5)
