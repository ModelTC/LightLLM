...
def test_extract_indexer_ks():
    ...
    k_fp8_original = (
        (k_bf16_original / k_abs_max).clamp(torch.finfo(fp8_type).min, torch.finfo(fp8_type).max).to(fp8_type)
    )
    ...
    fp8_match = torch.allclose(k_fp8_extracted.to(torch.float32), k_fp8_original.to(torch.float32), atol=0, rtol=0)

    # Verify scales match
    scale_match = torch.allclose(k_scale_extracted, k_scale_original.squeeze(-1), atol=1e-6, rtol=1e-5)
    ...
    k_fp8_large = (
        (k_bf16_large / k_abs_max_large).clamp(torch.finfo(fp8_type).min, torch.finfo(fp8_type).max).to(fp8_type)
    )
    ...
    fp8_match_large = torch.allclose(k_fp8_ext_large.to(torch.float32), k_fp8_large.to(torch.float32), atol=0, rtol=0)
    scale_match_large = torch.allclose(k_scale_ext_large, k_scale_large.squeeze(-1), atol=1e-6, rtol=1e-5)
