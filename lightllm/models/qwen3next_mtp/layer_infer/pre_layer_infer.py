...
class Qwen3NextMTPPreLayerInfer(LlamaPreLayerInfer):
    ...
    def _mtp_forward(
        ...
        gemma_rmsnorm_forward(
            input_embdings, layer_weight.pre_fc_norm_embedding_weight_, self.eps_, out=input_embdings_normed
        )

        # Normalize hidden state
        tgt_embdings_normed = self.alloc_tensor(tgt_embdings.shape, tgt_embdings.dtype)
        gemma_rmsnorm_forward(tgt_embdings, layer_weight.pre_fc_norm_hidden_weight_, self.eps_, out=tgt_embdings_normed)
