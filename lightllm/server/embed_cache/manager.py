...
class CacheServer(rpyc.Service):
    ...
    def exposed_alloc(self, batch_md5_token_nums: bytes) -> bytes:
        """
    ...
    def exposed_release(self, ids_blob: bytes) -> None:
        ids = pickle.loads(ids_blob)
        ids = [obtain(id) for id in ids]
        return self._impl.release(ids)

    def exposed_set_items_data(self, ids_blob: bytes) -> bytes:
        ids = pickle.loads(ids_blob)
        ids = [obtain(id) for id in ids]
        status_list = self._impl.set_items_data(ids)
        return pickle.dumps(status_list)

    def exposed_get_items_data(self, ids_blob: bytes) -> bytes:
        ids = pickle.loads(ids_blob)
        ids = [obtain(id) for id in ids]
        status_list = self._impl.get_items_data(ids)
        return pickle.dumps(status_list)

    def exposed_set_items_embed(self, ids_blob: bytes) -> None:
    ...
    def exposed_get_items_embed(self, ids_blob: bytes) -> bytes:
        ids = pickle.loads(ids_blob)
        ids = [obtain(id) for id in ids]
