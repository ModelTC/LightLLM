...
async def chat_completions_impl_v2(request: ChatCompletionRequestV2, raw_request: Request) -> Response:
    ...
    modalities = getattr(request, "modalities", ["text"])
...
async def _resize_to_user_request(images: List[Union[str, bytes]], r_width: int, r_height: int, t_width: int, t_height: int):
    if r_width > -1 and r_height > -1:
        if r_width != t_width or r_height != t_height:
            logger.info(f"resize image to user request size ({r_width}x{r_height}) from x2i server generated image size ({t_width}x{t_height})")

            def _resize(image: Union[str, bytes]):
                bytes_io = BytesIO(image) if isinstance(image, bytes) else BytesIO(base64.b64decode(image))
                image = Image.open(bytes_io)
                original_format = image.format
                assert image.width == t_width and image.height == t_height
                image = image.resize((r_width, r_height), Image.Resampling.LANCZOS)
                buffer = BytesIO()
                image.save(buffer, format=original_format)
                return base64.b64encode(buffer.getvalue()).decode("utf-8")

            # this is a blocking operation, so we need to use asyncio.to_thread to run it in a separate thread
            return await asyncio.gather(
                *[asyncio.to_thread(_resize, image) for image in images]
            )

    return images


async def _images_to_image_generation_response(
    images: List[Union[str, bytes]], request: ImageGenerationRequest, x2i_params: X2IParams, output_image_tokens: int
) -> ImageGenerationResponse:
    r_width, r_height = request.img_config.width, request.img_config.height
    t_width, t_height = x2i_params.width, x2i_params.height
    images = await _resize_to_user_request(images, r_width, r_height, t_width, t_height)
    return ImageGenerationResponse(
        data=[ImageB64(b64_json=_raw_image_to_data_url(image, request.output_format)) for image in images],
        output_format=request.output_format,
        size=f"{t_width}x{t_height}" if r_width == -1 or r_height == -1 else f"{r_width}x{r_height}",
...
async def image_generation_impl(request: ImageGenerationRequest | ImageEditRequest, raw_request: Request) -> Response:
    ...
    if len(images) == 0:
        return create_error_response(HTTPStatus.INTERNAL_SERVER_ERROR, "No image generated by x2i server")

    output_image_tokens = g_objs.httpserver_manager.tokenizer.get_image_token_length_by_size(
        x2i_params.width, x2i_params.height
    )

    return await _images_to_image_generation_response(images, request, x2i_params, output_image_tokens)
