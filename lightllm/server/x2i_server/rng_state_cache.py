...
def capture_rng_state() -> Dict:
    ...
    if runtime.is_available():
        state["torch_accelerator"] = (runtime.device_type, accelerator.get_rng_state_all())
    return state
...
def restore_rng_state(state: Dict) -> None:
    ...
    if accelerator_state is not None:
        device_type, rng_states = accelerator_state
        runtime = get_backend().runtime
        if runtime.device_type == device_type and runtime.is_available():
            getattr(torch, device_type).set_rng_state_all(rng_states)
