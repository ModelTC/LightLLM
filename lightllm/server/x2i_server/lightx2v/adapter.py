...
class LightX2VServer:
    def __init__(self, args: StartArgs, rank: int, world_size: int, nccl_port: int):
        ...
        logger.info(
            f"set {runtime.device_type} device for x2v server {rank}/{world_size}; "
            f"visible devices: {os.environ.get('ASCEND_RT_VISIBLE_DEVICES') or os.environ.get('CUDA_VISIBLE_DEVICES')}"
        )
        runtime.set_device(self.rank)
        set_current_device_id(self.rank)

        # Shared host memory registration must happen after selecting the worker device.
    ...
    async def _process(self, param: X2IParams):
        ...
        scheduler = self.pipe.runner.scheduler
        scheduler.infer_steps = param.steps
        scheduler.caching_records = [True] * param.steps
