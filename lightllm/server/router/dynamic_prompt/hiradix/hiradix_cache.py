...
class HiRadixCache(RadixCache):
    def __init__(self, unique_name, total_token_num, rank_in_node, mem_manager, radix_manager, radix_info_queue):
        ...
        self.local_cache_manager = LocalCacheManager(
            radix_manager=self.radix_manager, mem_manager=mem_manager, rank_in_node=rank_in_node
        )
        self.radix_info_queue = radix_info_queue
        self.is_hi_radix_cache = True
        self.disk_cache_match_count = SharedArray(
            f"{unique_name}_disk_cache_match_count_{rank_in_node}", (1,), dtype=np.int64
        )
        ...
        self.disk_cache_match_ratio = SharedArray(
            f"{unique_name}_disk_cache_match_ratio_{rank_in_node}", (1,), dtype=np.float32
        )
