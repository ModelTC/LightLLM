...
import torch
from lightllm.server.router.model_infer.mtp_speculative.utils import sample_and_verify
...
def sample_and_verify_dflash2_tokens(
    ...
    sampling_manager = backend.model.req_manager.req_sampling_params_manager
    verify_width = sampling_manager.mtp_verify_width
    assert verify_width == backend.max_draft_step + 1
    assert logits.shape[0] == req_num * verify_width, "DFlash2 requires fixed-width verification"
    assert len(run_reqs) == logits.shape[0]

    # All-greedy batches retain token equality and the regular sampler's
    # logprobs, without reading proposal distributions or consuming rejection RNG.
    if all(req.sampling_param.shm_param.top_k == 1 for req in run_reqs):
