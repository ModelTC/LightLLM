...
class PastKVCacheModule(object):
    def __init__(self, backend):
        ...
        self.runtime = self.backend.backend_runtime
        ...
        self.page_index_buffer = torch.empty(
            (1024 * LIGHTLLM_TOKEN_HASH_LIST_SIZE,),
            dtype=torch.int32,
            device=self.runtime.target_device(),
        )
    ...
    def need_sync_compute_stream(self) -> bool:
        ...
        if self.runtime.device_type == "npu":
            logger.info("PastKVCacheModule: synchronize the NPU compute stream before host offload.")
            return True
    ...
    def _start_kv_cache_offload(self, req: InferReq) -> Optional[TransTask]:
        ...
        with self.runtime.stream(g_infer_context.get_cpu_kv_cache_stream()):
            ...
            page_indexes = torch.tensor(
                req.shm_req.past_kv_cache_page_indexes.get_all(),
                dtype=torch.int32,
                device="cpu",
                pin_memory=self.runtime.device_type == "cuda",
            )
            ...
            if self.runtime.device_type == "npu":
                if cpu_kv_cache_scale is not None or gpu_kv_cache_scale is not None:
                    raise NotImplementedError("quantized NPU KV offload for X2I is not supported")
                offload_separate_kv_to_cpu_for_x2i(
                    token_indexes=token_indexes,
                    k_buffer=mem_manager.k_buffer,
                    v_buffer=mem_manager.v_buffer,
                ...
                device_page_indexes = self.page_index_buffer[start:end]
                device_page_indexes.copy_(page_indexes)
                offload_gpu_kv_to_cpu_for_x2i(
                    ...
                    page_indexes=device_page_indexes,
                    tp_index=self.backend.rank_in_dp,
                    tp_world_size=self.backend.dp_world_size,
                    grid_num=16,
                )
            sync_event = self.runtime.create_event()
            sync_event.record()
