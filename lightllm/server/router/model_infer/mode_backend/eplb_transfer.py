...
class NixlEPLBTransfer(_EPLBTransferBase):
    ...
    def _release_xfers(self, xfers) -> None:
        for local_dlist, remote_dlist, xfer in xfers:
            for handle, release in (
                (xfer, self._nixl_agent.release_xfer_handle),
                (remote_dlist, self._nixl_agent.release_dlist_handle),
                (local_dlist, self._nixl_agent.release_dlist_handle),
            ):
    ...
    def shutdown(self) -> None:
        ...
        if agent is not None:
            for xfer in self._xfer_cache.values():
                try:
