...
def padded_prepare_prefill_inputs(
    ...
    for _ in range(padded_req_num):
        ...
        max_kv_seq_len = max(max_kv_seq_len, 1)
        max_cache_len = max(max_cache_len, 0)
        prefix_total_token_num += 0
...
def padded_prepare_eagle_decode_inputs(
    req_objs: List[InferReq], padded_req_num: int = 0, mtp_step: int = 0
) -> Tuple[ModelInput, List[InferReq], int]:
    run_reqs = []
    total_token_num = 0
    max_len_in_batch = 0
    ...
    need_mem_index_num = (b_seq_len.shape[0] - padded_req_num) * (mtp_step - 1)

    # dynamic prompt cache 准备 token
    g_infer_state_lock.acquire()
    if g_infer_context.radix_cache is not None:
        g_infer_context.radix_cache.free_radix_cache_to_get_enough_token(need_mem_index_num)
    mem_indexes = g_infer_context.req_manager.mem_manager.alloc(need_mem_index_num)
    g_infer_state_lock.release()
