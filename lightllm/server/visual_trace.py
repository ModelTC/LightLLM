...
def visual_trace_dump_dir_from_env() -> Optional[Path]:
    ...
    if raw_enabled not in {"1", "true", "yes", "on"}:
        raise ValueError(
            f"{TRACE_DUMP_ENV} must be one of 1/true/yes/on or 0/false/no/off"
        )
    raw_dir = os.getenv(TRACE_DUMP_DIR_ENV, DEFAULT_TRACE_DUMP_DIR).strip()
    if not raw_dir:
        raise ValueError(
            f"{TRACE_DUMP_DIR_ENV} must not be empty when tracing is enabled"
        )
...
class VisualTraceRecorder:
    ...
    def __init__(
        self, trace_id: str, directory: Optional[Path], request: dict[str, Any]
    ):
    ...
    def _write_json(self) -> None:
        ...
        temporary_path = self.directory / (
            f".{self._filename}.tmp-{os.getpid()}-{id(self)}"
        )
...
_main_model_trace: contextvars.ContextVar[
    Optional[tuple[VisualTraceRecorder, str, int]]
] = contextvars.ContextVar("lightllm_visual_main_model_trace", default=None)


@contextmanager
def bind_main_model_trace(
    recorder: VisualTraceRecorder, choice_trace_id: str, step: int
) -> Iterator[None]:
