...
def _input_content_to_chat_content(parts: List[Dict[str, Any]]) -> Any:
    ...
    for part in parts:
        ...
        if ptype in ("input_text", "output_text", "text", "summary_text"):
            ...
            # file_id is intentionally NOT supported: there is no Files API,
            # and synthesising file:// from an untrusted client field would
            # let callers read arbitrary local paths via api_openai.py's
            # file:// handler.
...
class UnsupportedToolChoiceError(ValueError):
    """Raised when tool_choice references a hosted tool LightLLM doesn't host
...
def _responses_tool_choice_to_chat(tool_choice: Any) -> Any:
    ...
    Hosted tool selections (web_search, file_search, ...) raise so the caller
    can return a 400 — silently downgrading to "auto" produces confusing
    outputs for clients who asked for a specific tool.
    """
    ...
    if isinstance(tool_choice, dict):
        ...
        raise UnsupportedToolChoiceError(
            f"tool_choice type '{tool_choice.get('type')}' is not supported by LightLLM; "
            "use 'auto', 'none', 'required', or {type:function, name:...}"
        )
    raise UnsupportedToolChoiceError(f"tool_choice must be a string or dict, got {type(tool_choice).__name__}")
...
def _responses_to_chat_request(body: Dict[str, Any]) -> Tuple[Dict[str, Any], str]:
    ...
    # response_format: Responses has two shapes — top-level `response_format`
    # (identical to Chat) and the newer `text.format` (which omits the
    # outer wrapper). Top-level wins when both are present.
    response_format = body.get("response_format")
    if response_format is None:
        text_block = body.get("text") or {}
        text_format = text_block.get("format") if isinstance(text_block, dict) else None
        if isinstance(text_format, dict):
            ...
            if fmt_type == "json_schema":
                response_format = {
                    "type": "json_schema",
                    "json_schema": {
                        k: text_format[k] for k in ("name", "description", "schema", "strict") if k in text_format
                    },
                }
    ...
    if response_format is not None:
        chat_dict["response_format"] = response_format
        ...
        for line in raw_chunk.split("\n"):
            ...
            for tc in delta.get("tool_calls") or []:
                ...
                if state["started"] and new_args:
                    # If another tool call is currently open, close it and
                    # reopen this one. Responses forbids deltas against a
                    # non-current item_id.
                    if current_tool_idx is not None and current_tool_idx != tc_idx:
                        for ev in _close_tool_item(current_tool_idx):
                            yield ev
                        # Reopen this one as a fresh function_call item so the
                        # stream stays valid. We allocate a new output_index
                        # and fc_id to match the "added -> delta(s) -> done"
                        # lifecycle Responses clients expect.
                        state["fc_id"] = _new_fc_id()
                        state["output_index"] = output_index
                        output_index += 1
                        state["closed"] = False
...
def _stateless_lifecycle_error(op: str) -> JSONResponse:
    """Return a Responses-shaped error for GET/DELETE/cancel on a stateless
    server. Retrieve and delete 404; cancel 400 (nothing to cancel)."""
    if op == "cancel":
        status = HTTPStatus.BAD_REQUEST
        message = (
            "LightLLM does not persist responses; there is nothing to cancel. "
            "If you need in-flight cancellation, close the streaming HTTP connection."
        )
    else:
        status = HTTPStatus.NOT_FOUND
        message = (
            "LightLLM serves /v1/responses statelessly; responses are not persisted "
            f"and cannot be {op}d. Re-issue the original request instead."
        )
    return _responses_error_response(status, message)
