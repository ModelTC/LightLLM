...
def copy_buffer_p2p(
    ...
    if len(src_buffer.shape) == 3:
        ...
        # 5D case: Use vectorized PyTorch operations (most efficient for 5D)
        layer_num, buffer_size, d1, d2, d3 = src_buffer.shape

        src_idx_expanded = src_indexes.view(1, -1, 1, 1, 1).expand(layer_num, -1, d1, d2, d3)
        dst_idx_expanded = dst_indexes.view(1, -1, 1, 1, 1).expand(layer_num, -1, d1, d2, d3)

        src_data = torch.gather(src_buffer, 1, src_idx_expanded)
        dst_buffer.scatter_(1, dst_idx_expanded, src_data)
...
def copy_buffer_broadcast(
    ...
    if len(src_buffer.shape) == 3:
        ...
        dst_buffer.scatter_(
            1,
            dst_idx_expanded.reshape(layer_num, -1, d1, d2, d3),
            src_data_broadcast.reshape(layer_num, -1, d1, d2, d3),
        )
