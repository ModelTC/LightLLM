...
def get_run_key(q, max_kv_seq_len):
    ...
    # 正常查找使用 Graph 改写长度上限前保存的真实 KV 长度，不读取 GPU 张量或请求表容量。
    max_kv_seq_len = int(max_kv_seq_len)
    if Autotuner.is_kernel_autotune_warmup(AutotuneKernelType.DECODE_ATTENTION) and get_triton_autotune_level() in [
        ...
        max_kv_seq_len = get_decode_attn_autotune_seq_len()
    # 调优和正常查找统一按 512 token 向上分桶；实际 benchmark 保留环境变量指定的精确长度。
    max_kv_seq_len = (max_kv_seq_len + 511) // 512 * 512
...
def rebuild_inputs(
    ...
    **kwargs,
    ...
    # Graph 初始化请求很短，请求表宽度又是容量上限，两者都不能代表实际 decode 的计算量。
    # 仅在实际搜索前重建一次输入，不计入 benchmark；正式执行和 Graph 捕获仍使用原始输入。
    batch_size = q.shape[0]
    max_kv_seq_len = get_decode_attn_autotune_seq_len()
    # int4 数据和 scale 按同一物理 token 索引访问，四个缓存的 token 容量必须一致。
    assert (
        k.shape[0] == v.shape[0] == k_scale.shape[0] == v_scale.shape[0]
    ), "K/V caches and scales must have the same number of tokens"
    num_tokens = k.shape[0]
    if num_tokens == 0:
        raise ValueError("Int4 KV decode autotuning requires a non-empty KV cache")

    # 新建已初始化的映射，不能只拉长 B_Seqlen 后访问原请求表中未填充的位置。
    # 物理容量充足时各请求使用不同 token，不足时取模复用已有 K/V 和 scale，保证索引不越界。
    # 复用可避免重新分配完整的量化缓存，但可能提高 GPU 缓存命中率，影响调优的访存特征。
    Req_to_tokens = torch.arange(batch_size * max_kv_seq_len, dtype=Req_to_tokens.dtype, device=Req_to_tokens.device)
    Req_to_tokens = Req_to_tokens.remainder_(num_tokens).view(batch_size, max_kv_seq_len)
    # 新表只有 batch_size 行，因此请求索引也需重建，不能继续使用原全局请求表的行号。
    B_req_idx = torch.arange(batch_size, dtype=B_req_idx.dtype, device=B_req_idx.device)
    B_Seqlen = torch.full_like(B_Seqlen, max_kv_seq_len)
...
@autotune(
    kernel_name="_fwd_kernel_flash_decode_stage1:v2",
