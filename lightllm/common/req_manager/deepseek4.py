...
from lightllm.common.kv_cache_mem_manager.deepseek4_mem_manager import DSV4_C4_PAGE_SIZE, DSV4_PROMPT_CACHE_PAGE_SIZE
...
class DeepseekV4PromptCachePayload:
    ...
    槽位与 compressor 状态都不进载荷；c4 compressor 状态随 swa 页
...
class DeepseekV4ReqManager(ReqManager):
    ...
    def __init__(
        ...
        self._swa_allocated_end = [0 for _ in range(max_request_num + 1)]
        return
    ...
    def prepare_prefill_swa(
        ...
        for req_idx, seq_len in zip(req_list, seq_list):
            self._swa_allocated_end[req_idx] = seq_len
        return
    ...
    def prepare_pd_decode_cache(
        ...
        for req_idx, swa_start, seq_len in zip(req_list, swa_start_list, seq_list):
    ...
    def prepare_decode_swa(
        ...
        new_rows = [
            ...
            if req_idx != self.HOLD_REQUEST_ID and seq_len > self._swa_allocated_end[req_idx]
        ]
        if not new_rows:
            return
        req_list = [req_list[i] for i in new_rows]
        seq_list = [seq_list[i] for i in new_rows]
        rows = torch.tensor(new_rows, dtype=torch.int64, device=mem_indexes.device)
        mem_indexes = mem_indexes[rows]
        if prev_mem_indexes is not None:
            prev_mem_indexes = prev_mem_indexes[rows]
    ...
    def finish_cpu_cache_load(self, req_idx: int, loaded_len: int) -> None:
        ...
        self._swa_allocated_end[req_idx] = loaded_len
        return
    ...
    def clear_runtime_state(self, req_idx: int):
        ...
        self._swa_allocated_end[req_idx] = 0
        return
    ...
    def free_all(self):
        ...
        self._swa_allocated_end = [0 for _ in range(self.max_request_num + 1)]
        return
