...
if [[ ! -f "${RUN_RESULTS_FILE}" ]]; then
    echo "timestamp,mode,ratio,mtp_step,dataset,run_index,samples,concurrency,throughput,avg_latency,avg_ttft,avg_inter_token_latency,mtp_avg_token_per_step,mtp_avg_verify_tokens_per_step" > "${RUN_RESULTS_FILE}"
fi
if [[ ! -f "${RESULTS_FILE}" ]]; then
    echo "timestamp,mode,ratio,mtp_step,dataset,selected_run,samples,concurrency,throughput,avg_latency,avg_ttft,avg_inter_token_latency,mtp_avg_token_per_step,mtp_avg_verify_tokens_per_step" > "${RESULTS_FILE}"
fi
...
kill_lightllm() {
    pkill -9 -f "lightllm::.*::api_server" 2>/dev/null || true
    sleep 1
    pkill -9 -f "lightllm::.*::metric_manager" 2>/dev/null || true
    sleep 1
    pkill -9 -f "lightllm::.*::detokenization_server" 2>/dev/null || true
    sleep 1
...
extract_mtp_metrics() {
    local log_file="$1"
    local helper_output mtp_avg_token_per_step mtp_avg_verify_tokens_per_step
...
for mode in "${MODE_LIST[@]}"; do
    ...
    kill_lightllm
    bash "${STARTUP_SCRIPT}" --port "${PORT}" --mtp-step "${MTP_STEP}" > "${SERVER_LOG}" 2>&1 &
    SERVER_PID=$!
    ...
    for run_index in $(seq 1 "${REPEATS}"); do
        ...
        if ! run_benchmark_with_retry "${BENCH_LOG}" "${DATASET_PATH}" "${mode}"; then
            ...
            RUN_LINE="${TIMESTAMP},${mode},${RATIO},${MTP_STEP},${DATASET_PATH},${run_index},${SAMPLES},${CONCURRENCY},FAILED,FAILED,FAILED,FAILED,FAILED,FAILED"
