...
This script assumes a LightLLM server is already running with MTP enabled and
that its server log is being written to a readable file. It tags every request
with a unique X-Request-Id, waits for the matching server log line, and parses
both `mtp_avg_verify_tokens_per_step` and `mtp_avg_token_per_step` from that log.

Example:
    python test/speculative/low_accept/select_low_accept_dataset.py \
        --server-log /tmp/lightllm_qwen3_32b.log \
        --model qwen/qwen3-32b \
        --candidate-datasets datasets/alpaca.json datasets/mmlu.json datasets/pubmedqa.json datasets/truthfulqa.json \
        --gsm8k-path datasets/gsm8k.json \
        --probe-samples-per-dataset 100 \
        --low-accept-count 64 \
        --output-dir /tmp/low_accept_gsm8k_mix
"""
...
def wait_for_log_metrics(log_path: Path, request_id: str, start_offset: int, timeout_s: float) -> Dict[str, float]:
    ...
    with log_path.open("r", encoding="utf-8", errors="ignore") as f:
        ...
        while time.time() < deadline:
            ...
            # The same request id can appear in multiple log lines, such as:
            # 1. "received req ..."
            # 2. final summary line containing mtp_avg_* metrics.
            # Skip earlier lines and keep waiting for the summary line.
