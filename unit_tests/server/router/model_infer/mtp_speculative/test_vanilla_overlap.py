...
def test_dp_vanilla_proposer_owns_overlap_decode():
    ...
    model_input0 = SimpleNamespace(batch_size=6, b_req_idx=torch.arange(6, dtype=torch.int32, device=device))
    model_input1 = SimpleNamespace(batch_size=6, b_req_idx=torch.arange(6, dtype=torch.int32, device=device))
    ...
    proposal = proposer.propose_next_overlap(
        ...
        target_next_token_ids=torch.tensor([10, 11, 0, 20, 21, 22], dtype=torch.int64, device=device),
        real_verify_rows0=3,
        real_verify_rows1=3,
