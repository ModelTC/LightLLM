...
def test_ineligible_fusion_does_not_allocate_norm_output(monkeypatch):
    ...
    layer._ffn_norm = lambda *_args: (_ for _ in ()).throw(
        AssertionError("fallback must preserve the existing fused add-RMSNorm path")
    )
    ...
    def add_rmsnorm(*, input, residual, eps, alloc_func):
        add_rmsnorm_calls.append((input, residual, eps, alloc_func))
        input.add_(residual)
        return input.clone()

    layer.eps_ = 1e-6
    layer_weight = SimpleNamespace(
        ffn_norm_weight_=SimpleNamespace(
            weight=torch.ones(4),
    ...
    assert len(add_rmsnorm_calls) == 1
