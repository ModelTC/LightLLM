...
def test_token_attention_nopad_fa3_fp8(batch, seqlen, q_heads, kv_heads, head_dim):
    ...
    k_cache = kv.view(-1, 2 * KV_HEADS, HEAD_DIM)[:, :KV_HEADS, :].contiguous()
    v_cache = kv.view(-1, 2 * KV_HEADS, HEAD_DIM)[:, KV_HEADS:, :].contiguous()
    o1 = flash_attn_with_kvcache(
        q=q,
        k_cache=k_cache.view(-1, 1, kv_heads, head_dim),
        v_cache=v_cache.view(-1, 1, kv_heads, head_dim),
    ...
    k_cache_paged = k_cache.reshape(-1, page_size, kv_heads, head_dim)
    v_cache_paged = v_cache.reshape(-1, page_size, kv_heads, head_dim)

    page_table_paged = torch.empty((batch_size, N_CTX // page_size), dtype=torch.int32, device="cuda")
    page_table_paged.copy_(req_to_page_indexs[b_req_idx, : N_CTX // page_size])

    o2 = flash_attn_with_kvcache(
        q=q,
        k_cache=k_cache_paged,
        v_cache=v_cache_paged,
        page_table=page_table_paged,
        cache_seqlens=infer_state.b_seq_len,
        cu_seqlens_q=q_starts,
        cu_seqlens_k_new=kv_starts,
        max_seqlen_q=1,
    ...
    assert torch.allclose(o1, o2, atol=1e-2, rtol=1e-2)
    cos_sim2 = F.cosine_similarity(o1, o2).mean()
    print(cos_sim2)
    assert cos_sim2.item() == 1
