...
def test_sigmoid_mul_matches_unfused_dtype_rounding(dtype, gate_width):
    if not torch.cuda.is_available():
        pytest.skip("CUDA is required")

    torch.manual_seed(2026)
    x = torch.randn((17, 6144), device="cuda", dtype=dtype) * 2
    gate = torch.randn((17, gate_width), device="cuda", dtype=dtype) * 3
