...
def test_decode_path_no_fused_gating(batch):
    """Decode path without fused gating (explicit g/beta tensors)."""
    ...
    state_old = ssm_state.clone()
    state_new = ssm_state.clone()
...
def test_mtp_verify_path_no_fused_gating(mtp_step):
    """MTP verify path without fused gating (explicit g/beta)."""
