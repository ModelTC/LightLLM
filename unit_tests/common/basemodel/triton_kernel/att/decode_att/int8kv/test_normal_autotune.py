...
from lightllm.common.basemodel.attention.triton import int8kv as state_module
...
from lightllm.common.basemodel.triton_kernel.att.decode_att.int8kv.normal import (
    int8kv_flash_decoding_stage1 as stage1_module,
    int8kv_flash_decoding as decode_module,
)
...
def autotune_environment(monkeypatch):
    ...
    monkeypatch.setattr(state_module, "enable_diverse_mode_gqa_decode_fast_kernel", lambda: False)
...
def make_inputs(
    device="cpu", head_dim=64, group_size=8, dtype=torch.bfloat16, token_count=17, block_num=2, scale_dtype=None
):
    # 与内存管理器一致：K/V 和 scale 分别来自共享存储中的非连续视图。
    kv = torch.randint(-127, 128, (token_count, 4, head_dim), dtype=torch.int8, device=device)
    scale = (torch.rand(token_count, 4, head_dim // group_size, device=device) * 0.006 + 0.002).to(scale_dtype or dtype)
    return dict(
        ...
        B_seq_len=torch.tensor([3, 2], dtype=torch.int32, device=device),
        max_len_in_batch=3,
        mid_out=torch.full((2, 8, block_num, head_dim), -100, dtype=dtype, device=device),
        mid_out_logsumexp=torch.full((2, 8, block_num), -100, dtype=torch.float32, device=device),
        block_seq=256,
    )
...
def rebuild(inputs):
    ...
    return inspect.signature(stage1_module.flash_decode_stage1.fn).bind(*args, **kwargs).arguments
...
def test_rebuild_preserves_int8_cache_and_scales(monkeypatch, token_count):
    ...
    assert rebuilt["B_seq_len"].tolist() == [17, 17]
...
def test_decode_passes_saved_length_to_stage1(monkeypatch):
    inputs = make_inputs()
    state = Int8kvTritonDecodeAttState(
    ...
    calls = []
    monkeypatch.setattr(decode_module, "flash_decode_stage1", lambda **kwargs: calls.append(kwargs))
    monkeypatch.setattr(decode_module, "flash_decode_stage2", lambda **kwargs: None)
    state.decode_att(
        inputs["q"],
...
def reference_attention(inputs):
    def dequant(quantized, scale):
        group_size = quantized.shape[-1] // scale.shape[-1]
        # int8 直接乘分组 scale，转换为 Q 的 dtype 后再用 FP32 SDPA 验证 attention。
        return (quantized.float() * scale.float().repeat_interleave(group_size, dim=-1)).to(inputs["q"].dtype).float()
...
def reduce_output(inputs):
    ...
    flash_decode_stage2(
        inputs["mid_out"], inputs["mid_out_logsumexp"], inputs["B_seq_len"], output, inputs["block_seq"]
    )
    return output
...
@pytest.mark.parametrize(
    "head_dim,group_size,dtype,scale_dtype",
    [
        (64, 8, torch.bfloat16, torch.bfloat16),
        (128, 32, torch.float16, torch.float16),
        (256, 8, torch.bfloat16, torch.bfloat16),
        (128, 8, torch.bfloat16, torch.float32),
    ],
)
def test_long_kv_matches_dequantized_fp32_reference(monkeypatch, kv_len, head_dim, group_size, dtype, scale_dtype):
    monkeypatch.setenv("LIGHTLLM_DECODE_ATTN_AUTOTUNE_SEQ_LEN", str(kv_len))
    inputs = rebuild(make_inputs("cuda", head_dim, group_size, dtype, token_count=2 * kv_len, scale_dtype=scale_dtype))
    inputs["B_seq_len"][1] -= 1
...
def test_full_autotune_and_graph_reuse(tmp_path, monkeypatch, kv_len):
    ...
    def checked_bench(*args, **kwargs):
        ...
        assert rebuilt["B_seq_len"].tolist() == [kv_len, kv_len]
