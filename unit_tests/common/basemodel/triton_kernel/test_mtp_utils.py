...
def test_trim_dynamic_mtp_model_input(monkeypatch):
    ...
    assert trimmed_input.multimodal_params == [{"images": [], "audios": []}] * 8
    ...
    assert torch.equal(
        trimmed_input.b_position_delta.cpu(),
    ...
    assert torch.equal(
        trimmed_input.mem_indexes.cpu(), torch.tensor([100, 101, 102, 104, 108, 109, 110, 111], dtype=torch.int32)
    )
    ...
    assert torch.equal(trimmed_input.mem_indexes_cpu, torch.arange(12, dtype=torch.int32) + 100)
