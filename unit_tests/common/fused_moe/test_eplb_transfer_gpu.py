...
def _run_layers(transfer, control_group, layer_plans, callback=lambda layer_index: None):
...
def _eplb_worker(rank, port, queue):
    ...
    payload = layer_count * 16 * row_payload
    ...
    layer_plans = [(layer_index, plan) for layer_index in range(layer_count)]
    _run_layers(transfer, control_group, layer_plans)
    torch.cuda.synchronize()
