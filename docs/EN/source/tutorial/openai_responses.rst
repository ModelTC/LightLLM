...
Both shapes of structured-output selector are honoured and passed through to
the chat pipeline:

* Top-level ``response_format`` — the Chat Completions shape,
  e.g. ``{"type": "json_schema", "json_schema": {...}}``.
* ``text.format`` — the Responses-native shape,
  e.g. ``{"type": "json_schema", "name": "Ans", "schema": {...}}``.

If both are set, the top-level value wins. ``text.format.type`` values
``"json_schema"``, ``"json_object"``, and ``"text"`` are recognised.

Stateless lifecycle endpoints
-----------------------------

LightLLM serves ``/v1/responses`` statelessly — responses are not persisted.
Lifecycle endpoints return explanatory error envelopes rather than bare 404
HTML pages:

* ``GET /v1/responses/{id}`` — 404 ``not_found_error``.
* ``DELETE /v1/responses/{id}`` — 404 ``not_found_error``.
* ``POST /v1/responses/{id}/cancel`` — 400 ``invalid_request_error``. To
  cancel an in-flight stream, close the HTTP connection.

Clients like the OpenAI Agents SDK that call ``client.responses.retrieve(id)``
will see a clean error rather than a bare 404.
