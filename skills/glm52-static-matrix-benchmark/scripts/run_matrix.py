...
def base_command(
    ...
    if mtp:
        cmd += [
            ...
            "2",
            "--mtp_draft_model_dir",
            os.path.expanduser("~/models/GLM-5.2/"),
            "--mtp_accept_rate",
            "0.85",
        ]
    return cmd
    ...
    try:
        for rank in launch_order:
            ...
            if rank == 0:
                ...
                remote_command = f"cd {shlex.quote(str(root))} && exec env {exports} " f"{shlex.join(commands[rank])}"
                proc = subprocess.Popen(
                    [
                        "ssh",
                        "-o",
...
def run_case(args, stage: str, contexts, hits, max_batch_size=None):
    ...
    suffix = (
        f"prefill_ctx{'-'.join(map(str, contexts))}_hit{hit_suffix}{config_suffix}"
        if stage == "prefill"
        else f"decode_cap{max_batch_size}_ctx{'-'.join(map(str, contexts))}{config_suffix}"
    )
    stem = out_dir / suffix
    parsed_path = stem.with_suffix(".json")
    log_paths = [
        stem.with_suffix(".log") if rank == 0 else Path(f"{stem}.rank{rank}.log") for rank in range(len(node_hosts))
    ]
        ...
        "nodes": [
            {
                ...
                "log": str(log_paths[rank]),
                "returncode": returncodes[rank],
            }
...
def main():
    ...
    raise SystemExit(
        run_case(args, "prefill" if args.action == "prefill" else "decode", contexts, hits, args.max_batch_size)
    )
